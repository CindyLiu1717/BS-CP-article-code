...
class CPADF:
    ...
    def get_logz(self, mu, v, y): #进来的值是mu_n, v_n, y_n
       ...
       #logZ = -torch.lgamma(prior_a) + torch.logsumexp(log_weight + log_f_node,dim=0)
        return logZ
        #r=1 pass #0, 12000 points, rmse = 0.550266   pass #1, 28001 points, rmse = 0.916833
...
def test_movielens():
    ind = []
    y = []
    with open('./data/movielens_1m/mv_train_ind.txt', 'r') as f:
        for line in f:
            items = line.strip().split(',')
        ind = np.loadtxt('./data/movielens_1m/mv_train_ind.txt').astype(int)
        y = np.loadtxt('./data/movielens_1m/mv_train_y2.txt').astype(int)
        #ind = np.array(ind)
        #y = np.array(y)
    nvec = np.max(ind, 0) + 1
    ...
    with open('./data/movielens_1m/mv_test_ind.txt', 'r') as f:
        for line in f:
            items = line.strip().split(',')
        test_ind = np.loadtxt('./data/movielens_1m/mv_test_ind.txt').astype(int)
        test_y = np.loadtxt('./data/movielens_1m/mv_test_y2.txt').astype(int)
...
if __name__ == '__main__': #当前文件被直接运行时，执行下面的代码；而当该文件作为模块被导入到其他文件中时，不执行下面的代码。
    test_movielens()         #在当前文件被直接运行时，执行`test_beijing()`函数
